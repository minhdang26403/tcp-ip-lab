...
Address::Address(const sockaddr *addr, const size_t size) : _size(size)
{
  ...
  if (size > sizeof(_address.storage)) { throw runtime_error("invalid sockaddr size"); }
...
Address::Address(const string &node, const string &service, const addrinfo &hints) : _size()
{
  ...
  if (resolved_address == nullptr) { throw runtime_error("getaddrinfo returned successfully but with no results"); }
...
pair<string, uint16_t> Address::ip_port() const
{
  ...
  const int gni_ret = getnameinfo(static_cast<const sockaddr *>(_address), _size, ip.data(), ip.size(), port.data(),
                                  port.size(), NI_NUMERICHOST | NI_NUMERICSERV);
  if (gni_ret != 0) { throw tagged_error(gai_error_category(), "getnameinfo", gni_ret); }
