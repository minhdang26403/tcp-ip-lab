...
class Parser
{
  class BufferList
  {
   ...
   public:
    ...
    {
      for (const auto& x : buffers) { append(x); }
    }
    ...
    {
      if (buffer_.empty()) { throw std::runtime_error("peek on empty BufferList"); }
    ...
    void dump_all(std::vector<Buffer>& out)
    {
      ...
      if (skip_) { first_str = first_str.substr(skip_); }
      out.emplace_back(std::move(first_str));
      buffer_.pop_front();
      for (auto&& x : buffer_) { out.emplace_back(std::move(x)); }
    }
    ...
    void dump_all(Buffer& out)
    {
      ...
      out.release().clear();
      for (const auto& s : concat) { out.release().append(s); }
    }
  ...
  {
    if (size > input_.size()) { error_ = true; }
  }
...
class Serializer
{
 ...
 public:
  ...
  {
    for (const auto& b : bufs) { buffer(b); }
  }
