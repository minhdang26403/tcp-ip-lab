...
void EthernetHeader::parse(Parser& parser)
{
  ...
  for (auto& b : dst) { parser.integer(b); }

  // read source address
  for (auto& b : src) { parser.integer(b); }
...
void EthernetHeader::serialize(Serializer& serializer) const
{
  ...
  for (const auto& b : dst) { serializer.integer(b); }

  // write source address
  for (const auto& b : src) { serializer.integer(b); }
