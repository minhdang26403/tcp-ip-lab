    ...
    : FileDescriptor(move(fd))
{
  ...
  if ((len != sizeof(actual_value)) or (actual_value != domain)) { throw runtime_error("socket domain mismatch"); }

  // verify type
  len = getsockopt(SOL_SOCKET, SO_TYPE, actual_value);
  if ((len != sizeof(actual_value)) or (actual_value != type)) { throw runtime_error("socket type mismatch"); }
...
void DatagramSocket::recv(Address &source_address, string &payload)
{
  ...
  if (recv_len > static_cast<ssize_t>(payload.size())) { throw runtime_error("recvfrom (oversized datagram)"); }
...
void Socket::throw_if_error() const
{
  ...
  if (len != sizeof(socket_error)) { throw runtime_error("unexpected length from getsockopt: " + to_string(len)); }
