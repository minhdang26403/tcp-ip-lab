...
string ARPMessage::to_string() const
{
  ...
  if (opcode == OPCODE_REQUEST) { opcode_str = "REQUEST"; }
  if (opcode == OPCODE_REPLY) { opcode_str = "REPLY"; }
...
void ARPMessage::parse(Parser& parser)
{
  ...
  for (auto& b : sender_ethernet_address) { parser.integer(b); }
  parser.integer(sender_ip_address);

  // read target addresses (Ethernet and IP)
  for (auto& b : target_ethernet_address) { parser.integer(b); }
...
void ARPMessage::serialize(Serializer& serializer) const
{
  ...
  // read sender addresses (Ethernet and IP)
  for (const auto& b : sender_ethernet_address) { serializer.integer(b); }
  serializer.integer(sender_ip_address);

  // read target addresses (Ethernet and IP)
  for (const auto& b : target_ethernet_address) { serializer.integer(b); }
