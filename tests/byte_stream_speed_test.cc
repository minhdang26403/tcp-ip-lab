                ...
                const size_t read_size)    // NOLINT(bugprone-easily-swappable-parameters)
{
  ...
  for (size_t i = 0; i < data.size(); i += write_size) { split_data.emplace(data.substr(i, write_size)); }
  ...
  while (not bs.reader().is_finished()) {
    ...
    if (bs.reader().bytes_buffered()) {
      ...
      if (peeked.empty()) { throw runtime_error("ByteStream::reader().peek() returned empty view"); }
      output_data += peeked;
  ...
  if (gigabits_per_second < 0.1) { throw runtime_error("ByteStream did not meet minimum speed of 0.1 Gbit/s."); }
}
