...
class Host
{
  ...
  {
    return ranges::any_of(_expecting_to_receive,
                          [&expected](const auto& x) { return equal(x, expected); });
  }
...
public:
  Network()
    : default_id(
      _router.add_interface({random_router_ethernet_address(), Address {"171.67.76.46"}}))
  ...
  void simulate_physical_connections()
  {
    ...
    exchange_frames(
      "router.eth0", _router.interface(eth0_id), "applesauce", host("applesauce").interface());
    exchange_frames(
